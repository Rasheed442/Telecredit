...
const carouselSlides = [
  {
    ...
    description: "Real-time fraud detection and behavioral analytics purpose-built for Nigerian financial institutions."
  },
  {
    image: loginbg,
    leftBadgeTitle: "100M+",
    ...
    description: "Advanced AI-driven security protecting millions of daily transactions across Africa."
  },
  {
    image: loginbg,
    leftBadgeTitle: "24/7",
    ...
    description: "Comprehensive reporting and analytics dashboard for fraud mitigation teams."
  }
];
          ...
          >
            <div className="relative w-full aspect-[16/10] rounded-2xl mb-8 group cursor-pointer">
              ...
              <div className="absolute -left-8 bottom-6 bg-[#1A2234]/30 backdrop-blur-md border border-white/20 rounded-full px-5 py-2.5 flex items-center gap-2 shadow-[0_8px_30px_rgb(0,0,0,0.12)] -rotate-3 group-hover:rotate-0 transition-transform duration-300 overflow-hidden">
                <span className="text-[#00E599] font-bold text-[16px]">
                </span>
            ...
            <div className="text-center flex flex-col items-center gap-4 w-full">
              <h2 className="text-[30px] text-white font-semibold font-sf-pro leading-tight">
                Airtime & Data Credit {" "}
                <span className="text-[#5490DE] font-dm-serif italic font-normal">Risk Control Hub</span>
              </h2>
              <p className="text-[#BEBEBE] text-[16px] font-light font-ibm-plex-sans text-center max-w-[90%]">
                Detect fraud, manage exposure, and enforce underwriting decisions across telco networks from a single command center.
              </p>
            </div>
            <div className="flex max-w-[650px] items-center gap-2 absolute bottom-[-10%]">
    ...
    <button
        ...
        <div className="flex flex-col items-center justify-center h-[90%]">
          <div className="w-full max-w-[600px] m-auto px-4">
            <h2 className="text-[#1B1B1B] text-[40px] text-gray-900 font-semibold font-sf-pro leading-18">Welcome back</h2>

            <p className="text-gray-500 font-ibm-plex-sans text-[15px] font-light">
              Enter your credentials to access system-level controls and<br/> analytics
            </p>

            <Formik
              key={loginMode}
              initialValues={{ email: '', username: '', password: '' }}
              validationSchema={loginMode === "admin" ? adminLoginSchema : orgLoginSchema}
            ...
            >
              {({ errors, touched, isSubmitting }) => (
                <Form className="flex flex-col gap-4 pt-4 py-4">
                    ...
                    <Field name="email">
                      {({ field }: any) => (
                        <LabelInput
                          ...
                          type="email"
                          style="bg-[#f7fbff]  w-full border-1 border-[#ECEFF3] border-[#9B9B9B] outline-none placeholder:text-[#9B9B9B] text-[#474747] placeholder:text-[16px] text-[16px] rounded-lg h-[44px] px-3 placeholder:font-light font-light"
                          labelstyle=""
                          field={field}
                        />
                      )}
                  ...
                  {loginMode === "admin" && errors.email && touched.email && (
                    <div className="text-red-500 text-xs mt-1">{errors.email}</div>
                  )}
                  {loginMode === "org" && errors.username && touched.username && (
            ...
            <div className="flex justify-center gap-1 items-center">
              <p className="text-[#1A1A1A] text-[14px] font-geist font-light">Forgot Password?</p>
              <p className="text-[#1A1A1A] text-[15px] font-geist font-normal border-b border-[#1A1A1A] cursor-pointer hover:opacity-80 transition-opacity">Reset Password</p>
